...
const Login = ({ setActivePage }) => {
  ...
  const submitLogin = async (event) => {
    ...
    setActivePage(result.role === 'super_admin' ? 'super-dashboard' : 'admin-dashboard');
  };
    ...
    <div className="relative flex min-h-screen items-center justify-center p-4 sm:p-6 md:p-10 bg-[#031E3C] overflow-hidden">
      ...
      <section className="relative z-10 flex w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-100 bg-white/98 backdrop-blur-sm shadow-2xl">
        ...
        <div className="w-full md:w-1/2 p-4 sm:p-5 flex flex-col justify-center">
          ...
          {message ? <div className="mb-3 rounded-2xl bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700">{message}</div> : null}

          <form onSubmit={submitLogin} className="space-y-3.5">
            <div>
              <label className="text-xs font-semibold text-white">Email</label>
              <input type="email" placeholder="admin@example.com" value={loginForm.email} onChange={(event) => setLoginForm({ ...loginForm, email: event.target.value })} className="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-[#00D6CC] transition" required />
            </div>
            <div>
              <label className="text-xs font-semibold text-white">Password</label>
              <input type="password" placeholder="••••••" value={loginForm.password} onChange={(event) => setLoginForm({ ...loginForm, password: event.target.value })} className="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-[#00D6CC] transition" required />
            </div>
            <button disabled={loading} className="w-full rounded-2xl bg-[#00D6CC] px-4 py-2.5 text-sm font-bold text-white hover:opacity-90 transition disabled:opacity-50">
          ...
          <div className="mt-6 text-center text-xs text-slate-400">
            ...
            <button
              ...
              className="font-bold text-[#00D6CC] hover:underline"
            >
              Register Company
            </button>
          </div>
