...
const MakersList = ({ setActivePage }) => {
  const { makers, updateMaker, deleteMaker } = useMakeModels();
  ...
  const filtered = useMemo(() =>
    makers.filter(m => m.name.toLowerCase().includes(search.toLowerCase())),
    [makers, search]
  );
  ...
  const handleSaveEdit = () => {
    if (editName.trim()) {
      updateMaker(editingId, { name: editName.trim() });
    }
    setEditingId(null);
    setEditName('');
  };

  const handleToggleStatus = (maker) => {
    updateMaker(maker.id, { status: maker.status === 'active' ? 'inactive' : 'active' });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-bold text-slate-950 sm:text-4xl">Makers List</h2>
        </div>
        <button
          onClick={() => setActivePage('add-new-maker')}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 px-6 py-4">
          <div className="relative w-full max-w-xs">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm outline-none transition"
              onFocus={e => e.target.style.borderColor = '#00D6CC'}
              onBlur={e => e.target.style.borderColor = '#e2e8f0'}
            />
          </div>
          <span className="text-xs font-medium text-slate-400">
            {filtered.length} makers found
          </span>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {paginated.map((maker, idx) => (
                <tr key={maker.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    {editingId === maker.id ? (
                      <input
                        ...
                        className="rounded-xl border px-3 py-1.5 text-sm outline-none w-48"
                  ...
                  <td className="px-6 py-4">
                    <button
                      onClick={() => handleToggleStatus(maker)}
                    ...
                    >
                      {maker.status === 'active' ? 'ACTIVE' : 'INACTIVE'}
                    </button>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    {deleteConfirmId === maker.id ? (
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          onClick={() => { deleteMaker(maker.id); setDeleteConfirmId(null); }}
