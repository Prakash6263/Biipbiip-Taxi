...
import { UploadCloud, LayoutGrid, Table, ChevronDown, ChevronUp } from 'lucide-react';
        ...
        viewMode === 'card' ? (
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            {filteredRequests.map((request) => {
              ...
              return (
                <article key={request.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h3 className="text-xl font-bold text-slate-950">{request.customerName}</h3>
                      <p className="text-sm text-slate-500">{request.customerEmail} • {request.customerPhone}</p>
                    </div>
                    <Badge status={request.status} />
                  </div>

                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    <Info label="Car" value={car?.name || 'Deleted car'} />
                    <Info label="Amount" value={currency(amount)} />
                    <Info label="Pickup Date" value={request.pickupDate} />
                    <Info label="Return Date" value={request.returnDate} />
                    <Info label="Location" value={request.pickupLocation} />
                    <Info label="Requested At" value={formatDate(request.createdAt)} />
                  </div>

                  <div className="mt-5">
                    <h4 className="mb-2 text-sm font-bold text-slate-950">User Documents</h4>
                    <FilePreview files={request.userDocuments} />
                  </div>

                  <div className="mt-5 rounded-2xl border border-slate-200 p-4">
                    <h4 className="font-bold text-slate-950">Upload documents after office verification</h4>
                    <p className="mt-1 text-sm text-slate-500">Driving license, Aadhaar, address proof jaise documents upload karein.</p>
                    ...
                    <textarea
                      ...
                      className="mt-3 min-h-20 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-slate-950"
                    />
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    <button
                      ...
                      className="rounded-2xl bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300"
                    >
                      Approve & Rent Car
                    </button>
                    <button
                      ...
                      className="rounded-2xl bg-rose-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:bg-slate-300"
                    >
                      Reject
                    </button>
                    <button
                  ...
                  {!request.userDocuments?.length && request.status === 'pending' ? (
                    <p className="mt-3 text-xs font-semibold text-amber-700">Approve button document upload ke baad enable hoga.</p>
                  ) : null}
                </article>
              );
            })}
